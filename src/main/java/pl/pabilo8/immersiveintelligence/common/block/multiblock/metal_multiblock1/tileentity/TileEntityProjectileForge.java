...
import pl.pabilo8.immersiveintelligence.Config.IIConfig.Machines.ProjectileForge;
...
import pl.pabilo8.immersiveintelligence.client.render.multiblock.metal.ProjectileForgeRenderer;
 ...
 * @since 04.03.2021
 */
public class TileEntityProjectileForge extends TileEntityMultiblockMetal<TileEntityProjectileForge, IMultiblockRecipe> implements ISoundTile, IGuiTile, IAdvancedSelectionBounds, IAdvancedCollisionBounds, IUpgradableMachine, IBooleanAnimatedPartsBlock, IConveyorAttachable, IDataDevice
{
	...
	public FluidTank[] tanksFactory = new FluidTank[]{new FluidTank(ProjectileForge.coolantTankCapacity)};
	public FluidTank[] tanksFiller = new FluidTank[]{new FluidTank(ProjectileForge.componentTankCapacity)};
	...
	public TileEntityProjectileForge()
	{
		super(MultiblockProjectileForge.INSTANCE, MultiblockProjectileForge.INSTANCE.getSize(), ProjectileForge.energyCapacity, true);
	}
	...
	public void update()
	{
		...
		if(fillerUpgrade) //filling component storage from inventory/tanks
		{
			...
			{
				if(componentInside.amount < ProjectileForge.componentCapacity)
				{
				...
				if(productionProgress <= 0)
				{
					if(!inventory.get(0).isEmpty()&&!componentInside.isEmpty())
					{
						...
						{
							productionProgress = (int)(ProjectileForge.fillingTime+ProjectileForge.fillingTime*0.3*bullet.getCaliber());
			...
			if(productionProgress <= 0)
			{
				if(!inventory.get(0).isEmpty())
				{
					...
					if(first.isPresent()&&inventory.get(0).getCount() >= producedBullet.getCoreMaterialNeeded())
					{
						...
						productionProgress = (int)(ProjectileForge.productionTime+ProjectileForge.productionTime*0.3*bullet.getCaliber());
	...
	{
		TileEntityProjectileForge master = this.master();
		if(master!=null)
		{
	...
	{
		TileEntityProjectileForge master = master();
	...
	@Override
	public TileEntityProjectileForge getUpgradeMaster()
	{
		return master();
	}
	...
	{
		ProjectileForgeRenderer.renderWithUpgrades(upgrades);
	}
	...
	public static class BulletComponentStack
	{
		...
		{
			return amount/(float)ProjectileForge.componentCapacity;
		}

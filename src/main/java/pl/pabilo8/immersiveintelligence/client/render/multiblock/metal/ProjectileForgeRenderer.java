...
import pl.pabilo8.immersiveintelligence.client.model.multiblock.metal.ModelProjectileForge;
 ...
 * @since 21-06-2019
 */
public class ProjectileForgeRenderer extends TileEntitySpecialRenderer<TileEntityProjectileForge> implements IReloadableModelContainer<ProjectileForgeRenderer>
{
	private static final ResourceLocation TEXTURE = new ResourceLocation(ImmersiveIntelligence.MODID+":textures/blocks/multiblock/projectile_forge.png");

	static RenderItem renderItem = ClientUtils.mc().getRenderItem();

	private static IConveyorBelt con;
	private static ModelProjectileForge model, modelFlipped;

	@Override
	public void render(TileEntityProjectileForge te, double x, double y, double z, float partialTicks, int destroyStage, float alpha)
	{
		if(te!=null&&!te.isDummy())
		{
			...
			ModelProjectileForge modelCurrent = te.mirrored?modelFlipped: model;
	...
	{
		model = new ModelProjectileForge();
		model.flipAllX();
		...
		modelFlipped = new ModelProjectileForge();
